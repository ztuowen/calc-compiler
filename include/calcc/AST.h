...
namespace calcc {
  namespace ast {
    ...
    private:
      ...
      Expr *body;
    public:
      FDecl(std::string iname, VAL_TYPE ivt, std::vector<VDecl *> &iparams, Expr *ibody) : Decl(iname, ivt),
                                                                                           params(iparams),
                                                                                           body(ibody) {}
    ...
    //! Variable Scope
    class VScope : public Expr {
    private:
      VAL_TYPE vt;
      VDecl *d;
      Expr *e;
    public:
      VScope(VDecl *id, Expr *ie) : d(id), e(ie) { vt = VAL_UNKNOWN; }

      virtual EXPR_TYPE getExprType() { return EXPR_VSCO; }

      virtual VAL_TYPE getValType() { return e->getValType(); }

      VDecl *getDecl() { return d; }
